...
class MaxLengthPlusGirthConstraintTest {
    MaxLengthPlusGirthConstraint maxLengthPlusGirthConstraint;

    @BeforeEach
    void setUp() {
        maxLengthPlusGirthConstraint = new MaxLengthPlusGirthConstraint(new Measure<>(BigDecimal.valueOf(360), DistanceUnit.CENTIMETER));
    }

    @Test
    void a_parcel_with_length_plus_girth_within_the_maximum_should_satisfy_the_constraint() {
        // arrange
    ...
    @Test
    void a_parcel_with_length_plus_girth_exceeding_the_maximum_should_not_satisfy_the_constraint() {
        // arrange
        var parcel = new Parcel(
                UUID.randomUUID(),
                new Dimensions(
                        new Measure<>(BigDecimal.valueOf(60), DistanceUnit.CENTIMETER),
                        new Measure<>(BigDecimal.valueOf(100), DistanceUnit.CENTIMETER),
                        new Measure<>(BigDecimal.valueOf(60), DistanceUnit.CENTIMETER)
                ),
                mock(),
                Set.of(),
